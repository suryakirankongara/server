...
function Adddoctor() {
  ...
  useEffect(() => {
    fetch('http://localhost:3001/doctors')
      .then(response => response.json())
      .then(data => {
        // Update the doctor list with the retrieved data
        console.log(data);
      })
      .catch(error => {
              ...
              <div className="col-lg-8 offset-lg-2">
                <form id="adddoctorForm" name='adddoctorForm' onSubmit={adddoctor}>
                  ...
                  <div className="m-t-20 text-center">
                    <button id="adddoctor" type="submit" className="btn btn-primary submit-btn" style={{backgroundColor: '#b5344a', color: 'white', border: 'none'}}>Create Doctor</button>
                  </div>
                </form>
              </div>
            </div>
